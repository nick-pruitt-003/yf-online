...
export default function RulesTab({ handle, canEdit }: Props) {
  ...
  const numField = (label: string, value: number, onChange: (v: number) => void, min?: number, max?: number) => (
    <TextField
      ...
      type="number"
      value={value}
      onChange={(e) => { const parsed = Number(e.target.value); if (!Number.isNaN(parsed)) onChange(parsed); }}
      disabled={!editable}
      size="small"
  ...
  return (
    <Box display="flex" flexDirection="column" gap={3} maxWidth={700}>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" gutterBottom>Tossup Answer Types</Typography>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" gutterBottom>Round Length</Typography>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" gutterBottom>Bonus Settings</Typography>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" gutterBottom>Overtime</Typography>
          <Box display="flex" gap={3} flexWrap="wrap" alignItems="center">
            {numField(
              'Overtime tossups',
